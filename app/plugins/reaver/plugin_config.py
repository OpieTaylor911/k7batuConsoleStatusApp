#!/usr/bin/env python3
"""
Reaver Plugin Configuration

This module defines the plugin metadata for the Reaver WPS Attack Tool plugin.
"""
...
PLUGIN_NAME: str = "Reaver WPS Attack Tool"
PLUGIN_VERSION: str = "1.2.0"
REQUIRES_SUDO: bool = True
INSTALL_PACKAGES: List[str] = ["reaver", "pixiewps"]
SETUP_INSTRUCTIONS: str = """Reaver is a Brute Force attack tool against WiFi Protected Setup (WPS) pin.

Setup Steps:
1. Install reaver: sudo apt install reaver pixiewps
2. Put your wireless interface in monitor mode
3. Run the tool and select your target

Note: WPS attacks require:
- Wireless adapter capable of packet injection
- Target router with WPS enabled
- Patience (can take 5-10 minutes per attack)

Legal Notice: Only use on networks you own or have explicit permission to test.
"""

PERMISSION_COMMANDS: List[str] = [
    # Add reaver to sudoers for passwordless execution
    "echo 'bcaddy ALL=(root) NOPASSWD: /usr/bin/reaver' >> /etc/sudoers.d/reaver",
    "chmod 0440 /etc/sudoers.d/reaver"
]
...
if __name__ == "__main__":
    ...
    print(f"Install Packages: {', '.join(INSTALL_PACKAGES)}")
